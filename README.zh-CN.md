...
- Codex 只询问会实质影响实现的需求问题。
- 如果需求已经清晰，Codex 会记录假设并继续。
...
| `DELEGATE_CLARIFY` | `auto` | 需求澄清模式：`auto`、`always` 或 `never` |
...
- `@delegate-status` 输出当前 phase、marker、生成文件和下一步操作。
- `@delegate-recover` 尽可能恢复缺失状态，并把卡住的任务推进到下一个安全阶段。
